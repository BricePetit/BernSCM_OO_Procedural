 ...
 *
 * @return Return the created Ocean object.
 */
Ocean& createOceanModel(Ocean &ocean) {
    string doc;
    if (OCEAN_MODEL == "HILDA") {
        ...
        double ocean_t_scale[T_SCALE_SIZE] = {0.45253504e0, 2.19901724e0, 12.03837102e0, 59.58359820e0, 237.30651757e0,
                                              0.03855458e0, NA};
        Pirf ocean_pirf = Pirf(OCEAN_MODEL, 6, ocean_weight, ocean_t_scale);
        doc = "# HILDA ocean mixed layer pulse response\n" \
              ...
              "#\n" \
              "#\n";
        ocean = Ocean(ocean_pirf, 75e0, 4000e0, 1028e0, 1026.5e0, 3.62e14, 1e0 / 9.06e0, 18.1716e0, OCEAN_MODEL, doc);
        ...
        double ocean_t_scale[T_SCALE_SIZE] = {2.690038e0, 13.61728e0, 86.79685e0, 0.5762091e0, 337.2983e0, 0.07027151e0,
                                              NA};
        Pirf ocean_pirf = Pirf("Bern2D", 6, ocean_weight, ocean_t_scale);
        doc = "# Bern2.5D ocean mixed layer pulse response\n" \
              ...
              "#\n" \
              "#\n";
        ocean = Ocean(ocean_pirf, 50e0, 4000e0, 1028e0, 1026.5e0, 3.5375e14, 1e0 / 7.46e0, 18.2997e0, OCEAN_MODEL, doc);
        ...
        doc = "# Princeton AOGCM ocean effective mixed layer pulse response\n" \
              ...
              "#\n" \
              "#\n";
        ocean = Ocean(ocean_pirf, 50.9e0, 4000e0, 1028e0, 1026.5e0, 3.55e14, 1e0 / 7.66e0, 17.7e0, OCEAN_MODEL, doc);
    } else {
...
/**
 * Function to create the Land object.
 *
 * @return Return the created Land object.
 */
Land& createLandModel(Land &land) {
    string doc;
    if (LAND_MODEL == "HRBM") {
        ...
        double land_t_scale[T_SCALE_SIZE] = {0.2010730e0, 1.4753908e0, 8.889799331e0, 74.098061812e0, 2.538054e02, NA,
                                             NA};
        Pirf land_pirf = Pirf(LAND_MODEL, 5, land_weight, land_t_scale);
        ...
        Sirf land_sirf = Sirf(5e0, land_weight_sirf, land_t_scale_sirf);
        doc = "# HRBM pulse response/multibox substitute model\n" \
              ...
              "#\n" \
              "#\n";
        land = Land(land_pirf, land_sirf, 41.73282e0, 0e0, LAND_MODEL, doc);
        ...
        Pirf land_pirf = Pirf("4box", 4, land_weight, land_t_scale);
        doc = "# 4-box Biosphere of Bern Model\n" \
              ...
              "#\n" \
              "#\n";
        land = Land(land_pirf, 60e0, 0.287e0, LAND_MODEL, doc);
    } else {
...
int main() {
    double dt = DELTA_T, t2x, tot_time = 0;
    double **forcing{};
    bool t_dep, co2_dep;
    string scenario, id, input;
    Ocean ocean;
    Land land;
    ...
    UtilsFile utils = UtilsFile(id);
    forcing = utils.readForcing(forcing, scenario);
    for (int i = 0; i < 100; i++) {
        auto start = chrono::high_resolution_clock::now();
        atmosphere = Atmosphere();
        earth = Earth(createOceanModel(ocean), createLandModel(land), atmosphere, forcing, t2x, t_dep, co2_dep, scenario, dt);
